...
module.exports = ({ name, environment, options = {} }) => async () => {
  ...
  const contextService = services.find(
    ({ definition }) => definition.type == "context"
  );
  if (contextService) {
    await handleService(contextService);
  }
  let nonContextServices = services.filter(
    ({ definition }) => definition.type != "context"
  );
  console.log(options.only);
  if (options.only && options.only.length > 0) {
    nonContextServices = nonContextServices.filter(
      service => options.only.indexOf(toDotName(service)) != -1
    );
  }

  const servicesByDependency = nonContextServices
    .sort((left, right) => {
  ...
  const remainingServices = new Set(
    servicesByDependency.map(info => info.location.split("/").join("."))
  );
    ...
    return new Promise(async (resolve, reject) => {
      remainingServices.forEach(dotName => {
        ...
        const dependenciesFinished =
          hasDependencies &&
          dependencies.reduce((_, dependencyName) => {
            return !remainingServices.has(dependencyName) ? true : false;
          }, true);

        if (
          (dependenciesFinished || !hasDependencies) &&
          !inProgressServices.has(dotName)
        ) {
          ...
          thread.on("message", message => {
            if (message.command == "done") {
              remainingServices.delete(dotName) &&
                inProgressServices.delete(dotName);
              context.command.name == "start"
