...
const handleServiceWithContext = context => async ({ definition, location }, shouldRebuild) => {
  ...
  const mergeServiceContext = (serviceContext) => {
    if (definition.type == "context") {
      ...
      context.services[serviceDotName] = { ...existingContext, ...serviceContext };
    }
  }
  ...
  const installDependenciesForPackage = async (packageLocation) => {
    return new Promise( async (resolve, reject) => {
      ...
      if (context.command.name == 'install') {
        const dependencyStrings = lodash.map(packageJson.dependencies, (version, name) => `${name}@${version}`);
  ...
  serviceDotName = definition.type == "context" ? "context" : location.split('/').join('.');
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      packageLocation = resolveLocalPath(`./node_modules/unstack-${handlerName}`);
      ...
      try {
        outputProgressInfo(`Attempting to load local handler because ${err.message}`)
        packageLocation = resolveLocalPath(`./.unstack/handlers/${handlerName}`);
        const success = await installDependenciesForPackage(packageLocation);
        if (success) {
          handler = require(packageLocation);
        }
